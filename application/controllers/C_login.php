...
class C_login extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $data['CalendarAcademic'] = $this->db->get_where(
            'db_academic.calendar_academic',
            array('StatusPublish' => '2')
        )->result_array();
    ...
    public function forget_password()
    {
        ...
        $dataEmploy = $this->db->query('SELECT NIP, Name FROM db_employees.employees
                                                  WHERE Email = "' . $Email . '" 
                                                  OR EmailPU = "' . $Email . '" LIMIT 1')->result_array();

        if (count($dataEmploy) > 0) {

            $Name = $dataEmploy[0]['Name'];
            $Type = 'emp';
            ...
            $dataStudents = $this->db->select('NPM,Name')->get_where(
            ...
            if (count($dataStudents) <= 0) {
                ...
                $dataC = $this->db->select('Year')
                    ->order_by('ID', 'DESC')->get('db_academic.curriculum')->result_array();
        ...
        if (isset($_GET['code'])) {
            ...
            try {
                ...
                if (count($dataUser['url_direct']) > 0) {
                    ...
                    if ($To == 'std' || $To == 'emp') {
                        ...
                        if (count($dataSurvey) > 0) {
                            foreach ($dataSurvey as $item) {
                                ...
                                if (isset($item['emp_detail']) && count($item['emp_detail']) > 0) {

                                    if ($item['emp_detail'][0]['SurveyStatus'] == 1) {
                                        $checkSurvey = 0;
                                    }
                                }
                            }
                    ...
                    $data['Survey_checkSurvey'] = $checkSurvey;
                    $data['Survey_tokenDirectSurvey'] = $tokenDirectSurvey;
                    ...
                    $dataEm = $this->db->get_where(
                        'db_research.master_user_research',
                        array(
                            'Email' => $Email,
                        )
                    )->result_array();
    ...
    public function checkUsername()
    {
        ...
        if ($data_arr['userType'] != '' && $data_arr['userType'] == 'P') {
            ...
            if (count($dataStudents) > 0) {
                ...
                // Cek Apakah karyawan atau bukan
                $dataEmploy = $this->db->get_where(
                    'db_employees.employees',
                    array('NIP' => $data_arr['Username']),
                    1
                )->result_array();
    ...
    public function checkPassword()
    {
        ...
        if ($data_arr['User'] == 'Students') {
            ...
            if ($data_arr['Status'] == '-1') {
                $dataMhs = $this->db->get_where(
                    'db_academic.auth_students',
                    array('NPM' => $data_arr['Username'], 'Password_Old' => md5($data_arr['Password'])),
                    1
                )
                    ->result_array();
                ...
                $dataMhs = $this->db->get_where(
                    'db_academic.auth_students',
                    array('NPM' => $data_arr['Username'], 'Password' => $pass),
                    1
                )
                    ->result_array();
                ...
                if (count($dataMhs) > 0) {
                    ...
                    if ($DevelopMode == '1' && $data_arr['Password'] == $dataMode[0]['GlobalPassword']) {
                        $dataMhs = $this->db->get_where(
                            'db_academic.auth_students',
                            array('NPM' => $data_arr['Username']),
                            1
                        )
                            ->result_array();
                        $logon = $this->loadData_UserLogin(
                            'Students',
                            $dataMhs[0]['Year'],
                            $data_arr['Username'],
                            $data_arr['TypeUser']
            ...
            if ($data_arr['Status'] == '-1') {
                $dataEm = $this->db->get_where(
                    ...
                    array(
                        ...
                        'Password_Old' => md5($data_arr['Password'])
                    )
                )->result_array();
        ...
        if ($result['Status'] == 1 || $result['Status'] == '1') {
            ...
            if ($data_arr['User'] == 'Students' || $data_arr['User'] == 'Employees') {
                $result['Survey'] = $this->checkSurvey($data_arr['User'], $data_arr['Username'], $result['url_direct']);
            } else {
                $result['Survey'] = [];
            }
            ...
            // Insert Log Login pindahin ke log
            //            $hostname = gethostbyaddr($_SERVER['REMOTE_ADDR']);
            //            $dataLogLogin = array(
            //                'Username' => $data_arr['Username'],
            //                'UserType' => $UserType,
    ...
    public function checkSurvey($User, $Username, $url_direct)
    {
        ...
        if (count($dataCkSurvey) > 0) {
            ...
            // Data std
            $QueryUser = ($User == 'Students' || $User == 'std')
            ...
            for ($i = 0; $i < count($dataCkSurvey); $i++) {
                ...
                if ($checkExistFillOut > 0) {
                    ...
                    // Cek user mhs
                    if ($User == 'Students' || $User == 'std') {

                        $dataSurvDetail = $this->db->select('ID,TypeUser')
                            ->get_where(
                                'db_it.surv_survey_usr_std',
                        ...
                        if (count($dataSurvDetail) > 0) {
                            for ($s = 0; $s < count($dataSurvDetail); $s++) {
                                if ($dataSurvDetail[$s]['TypeUser'] == '1') {
                                    ...
                                    $dataSurvDetail[$s]['TypeUser'] == '2'
                                    && $dataUserSurv[0]['StatusStudentID'] == '3'
                                ) {
                                    $dataSurvDetail[$s]['SurveyStatus'] = 1;
                                } else if (
                                    $dataSurvDetail[$s]['TypeUser'] == '3'
                ...
                $UserType = ($User == 'Students' || $User == 'std') ? 'std' : 'emp';
        ...
        $result = (count($dataCkSurvey) > 0) ? $dataCkSurvey : [];
        return $result;
    }
    ...
    public function checkloginwithAd()
    {
        ...
        if ($total == 0) { // student 
            ...
            if ($total == 1) {
                ...
                $dataEmploy = $this->db->get_where(
                    'db_employees.employees',
                    array('EmailPU' => $Input['Username'] . '@podomorouniversity.ac.id'),
                    1
                )->result_array();
                if (count($dataEmploy) > 0) {
                    $DataUser = array(
